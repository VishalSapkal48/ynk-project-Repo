...
function MaterialChecklist({ language, toggleLanguage }) {
  ...
  const handleSubmit = async () => {
    ...
    checklistData.forEach((item) => {
      const particular =
        item[`particulars_${language}`] || item.particulars_en;
      const availability =
        materialAvailability[`${item.category}-${item.no}`] === "yes"
          ...
          ? "उत्तर दिले नाही"
          : "Not answered";
      formattedData.append(`${particular} (${item.category}-${item.no})`, availability); // युनिक की
    });
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#dbeeff]">
        <div className="bg-white p-6 rounded-xl shadow-md w-full max-w-md text-center">
          ...
          <h2 className="text-lg font-bold mb-2">
            {language === 'en' ? 'Checklist Submitted!' : 'चेकलिस्ट सबमिट झाले!'}
          </h2>
          ...
          >
            {language === 'en' ? 'Proceed to Inspection Checklist' : 'तपासणी चेकलिस्टवर जा'}
          </button>
        </div>
      </div>
    );
...
<div className="bg-white flex justify-between items-center mb-4 px-3 py-2 rounded">
        ...
        <div className="px-6 py-6">
          ...
          <div className="text-left text-gray-500 mb-4">
            <p>{config[language].branchName} {user.branch}</p>
            <p>{config[language].ownerName} {user.name}</p>
            <p>{config[language].dateLabel} {formatDateTime(language).replace(language === 'mr' ? 'दिनांक:' : 'Date:', '').trim()}</p>
            <p>{config[language].mobileLabel} {user.mobile}</p>
          </div>
          ...
          {error && (
            <p className="text-red-500 text-sm mt-4 text-center">
              {language === "mr"
                ? `त्रुटी: ${error?.data?.message || 'अज्ञात त्रुटी'}`
